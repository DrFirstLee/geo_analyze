# 🍊 Media Naranja GEO Analytics Console

![Project Status](https://img.shields.io/badge/Status-Active-brightgreen)
![Tech Stack](https://img.shields.io/badge/HTML5-TailwindCSS-orange)
![License](https://img.shields.io/badge/License-MIT-blue)

> **Media Naranja GEO Analytics Console** is a web-based diagnostic tool designed to assess how well a website is optimized for Generative AI search engines like ChatGPT, Gemini, and Perplexity.

---

### 🌐 Language / 언어 선택
- [🇺🇸 English Description](#-english-description)
- [🇰🇷 한국어 설명](#-한국어-설명)

---

## 🇺🇸 English Description

### 📖 Introduction
This project is the frontend client for the **GEO (Generative Engine Optimization) Analytics Service**. It allows users to input a website URL and receive a comprehensive score based on how AI-friendly the site's structure and content are.

The dashboard visualizes metrics such as **Technical Structure (JSON-LD)**, **Content Clarity**, and **Authority**, communicating with a FastAPI backend to retrieve real-time analysis data.

### ✨ Key Features
- **AI-Driven Audit:** Diagnoses site readability for LLMs (Large Language Models).
- **Interactive Dashboard:** Visualizes scores with animated charts and dynamic UI.
- **Multi-language Support (i18n):** Seamless toggle between English (EN) and Korean (KR).
- **Responsive Design:** Fully responsive UI built with Tailwind CSS.
- **Self-Optimized:** This application itself is optimized for GEO with embedded `JSON-LD` schemas and semantic HTML.

### 🛠 Tech Stack
- **Core:** HTML5, Vanilla JavaScript (ES6+)
- **Styling:** Tailwind CSS (via CDN for lightweight deployment)
- **Icons:** FontAwesome
- **Fonts:** Pretendard (Web Font)

### 🚀 Getting Started
This project is a static web application. No build process is required.

1. **Clone the repository**
   $$$bash
   git clone https://github.com/your-username/media-naranja-geo.git
   $$$

2. **Open the file**
   Simply open `index.html` in your modern web browser.

3. **Backend Connection**
   By default, the frontend is configured to connect to the Media Naranja API server.
   - API Endpoint: `http://49.166.98.88:5000/geo_analyze`
   - To change the backend URL, modify the `fetch` URL in the `analyzeSite()` function inside `index.html`.

### 📂 File Structure
$$$text
.
├── index.html       # Main application file (Frontend)
└── README.md        # Project documentation
$$$

---

## 🇰🇷 한국어 설명

### 📖 프로젝트 소개
이 프로젝트는 **GEO (생성형 엔진 최적화) 분석 서비스**의 프론트엔드 클라이언트입니다. 사용자가 웹사이트 URL을 입력하면, ChatGPT나 Gemini 같은 AI 모델이 해당 사이트를 얼마나 잘 이해하고 인용할 수 있는지 분석하여 점수화해 줍니다.

**기술적 구조(Technical)**, **콘텐츠 명확성(Content)**, **신뢰도(Authority)** 세 가지 핵심 지표를 시각화하여 보여주며, FastAPI 백엔드와 통신하여 실시간 진단 결과를 제공합니다.

### ✨ 주요 기능
- **AI 최적화 진단:** LLM(거대언어모델)이 읽기 쉬운 구조인지 분석 및 채점.
- **인터랙티브 대시보드:** 애니메이션 차트와 직관적인 UI로 분석 결과 시각화.
- **다국어 지원 (i18n):** 한국어(KR)와 영어(EN) 간 실시간 언어 전환 기능.
- **반응형 웹:** Tailwind CSS를 활용한 모바일/PC 완벽 대응.
- **자체 GEO 적용:** 이 애플리케이션 코드 자체에 `JSON-LD` 스키마와 시맨틱 태그가 적용되어 있어, GEO의 모범 사례를 보여줍니다.

### 🛠 기술 스택
- **Core:** HTML5, Vanilla JavaScript (ES6+)
- **Styling:** Tailwind CSS (CDN 활용)
- **Icons:** FontAwesome
- **Fonts:** Pretendard (프리텐다드)

### 🚀 실행 방법
이 프로젝트는 정적 웹 애플리케이션으로, 별도의 빌드 과정이 필요 없습니다.

1. **리포지토리 클론**
   $$$bash
   git clone https://github.com/your-username/media-naranja-geo.git
   $$$

2. **파일 실행**
   `index.html` 파일을 브라우저에서 바로 실행하세요.

3. **백엔드 연동**
   기본적으로 Media Naranja 테스트 서버와 통신하도록 설정되어 있습니다.
   - API 엔드포인트: `http://49.166.98.88:5000/geo_analyze`
   - 백엔드 주소를 변경하려면 `index.html` 내부 스크립트의 `analyzeSite()` 함수 내 `fetch` 주소를 수정하세요.

### 📂 폴더 구조
$$$text
.
├── index.html       # 메인 애플리케이션 (프론트엔드)
└── README.md        # 프로젝트 설명서
$$$

---

<p align="center">
  &copy; 2026 Media Naranja. All rights reserved.<br>
  <i>Empowering Brands in the Age of AI Search.</i>
</p>